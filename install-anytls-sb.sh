#!/usr/bin/env bash
# AnyTLS interactive installer for sing-box
# Author: ChatGPT
...
write_config() {
  local domain="$1" port="$2" user="$3" pass="$4" listen="$5" profile="$6"
...
open_firewall_port() {
  ...
  msg "防火墙规则已尝试添加。云厂商/NAT 面板仍需单独放行 TCP ${port}。"
}

save_info() {
  local domain="$1" port="$2" user="$3" pass="$4" profile="$5"
  local enc_pass uri insecure_text
  enc_pass="$(urlencode "$pass")"
  uri="anytls://${enc_pass}@${domain}:${port}#$(urlencode "$profile")"
  insecure_text="否"
...
install_anytls() {
  ...
  echo
  local domain port user pass listen profile
  domain="$(read_required "请输入绑定域名/SNI，例如 node.example.com")"
  port="$(read_default "请输入 AnyTLS TCP 监听端口" "443")"
  [[ "$port" =~ ^[0-9]+$ ]] || fatal "端口必须是数字。"
  [ "$port" -ge 1 ] && [ "$port" -le 65535 ] || fatal "端口范围必须是 1-65535。"
  ...
  write_config "$domain" "$port" "$user" "$pass" "$listen" "$profile"
  open_firewall_port "$port"
  restart_service
  save_info "$domain" "$port" "$user" "$pass" "$profile"
}
